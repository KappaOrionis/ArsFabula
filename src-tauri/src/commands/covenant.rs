...
pub async fn list_covenants(pool: State<'_, SqlitePool>) -> Result<Vec<Covenant>, String> {
    let covenants = sqlx::query_as::<_, Covenant>("SELECT * FROM covenants ORDER BY name ASC")
        .fetch_all(&*pool)
        .await
...
pub async fn create_covenant(
    ...
    sqlx::query(
        "INSERT INTO covenants (id, name, aura_type, aura_level, founding_year, tribunal) VALUES (?, ?, ?, ?, ?, ?)"
    )
    .bind(&id)
    .bind(name)
    .bind(aura_type)
